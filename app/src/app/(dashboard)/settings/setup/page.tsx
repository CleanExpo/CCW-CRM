...
export default function SetupPage() {
  ...
  const checkConnections = useCallback(async () => {
    ...
    try {
      ...
      setSteps((prev) =>
        prev.map((s) => (s.id === "cin7" ? { ...s, status: "pending" } : s))
      );
    }
    ...
    try {
      ...
      setSteps((prev) =>
        prev.map((s) => (s.id === "xero" ? { ...s, status: "pending" } : s))
      );
    }
    ...
    try {
      ...
      setSteps((prev) =>
        prev.map((s) =>
          s.id === "shopify" ? { ...s, status: "pending" } : s
        )
      );
    }
  }, []);
  ...
  const runInitialSync = async () => {
    ...
    try {
      ...
      toast({
        title: "Sync error",
        description:
          error instanceof Error ? error.message : "Failed to run initial sync",
        variant: "destructive",
      });
    } finally {
      setSyncing(false);
