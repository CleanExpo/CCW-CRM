/**
 * CCW-ERP-CRM — AI Front Desk configuration (SKELETON / EXAMPLE).
 *
 * Copy to `frontdesk.config.ts` and fill real values when the shared
 * `@nexus/front-desk` package lands. Ships dark until `CCW_FRONT_DESK_ENABLED=true`.
...
export interface FrontDeskConfig {
  /** Stable project key. */
  project: string;
  /** Enable flag — reads process.env.CCW_FRONT_DESK_ENABLED. Default off. */
  enabled: boolean;
...
export const config: FrontDeskConfig = {
  project: 'ccw',
  enabled: process.env.CCW_FRONT_DESK_ENABLED === 'true',
  brand: { assistantName: 'the CCW front desk', accent: '#000000' /* TODO brand accent */ },
  voice: { provider: 'elevenlabs', voiceId: 'TODO_CCW_VOICE_ID', locale: 'en-AU' },
  knowledge: { kind: 'prompt', source: 'TODO — this brand\'s knowledge base' },
  channels: ['phone-in', 'chat', 'phone-out', 'voice'],
  vendor: 'elevenlabs',
  phoneNumber: null,
  tools: ['crm', 'scheduling', 'quotes', 'human handoff'],
  compliance: {
    businessIdentity: 'CCW-ERP-CRM',
