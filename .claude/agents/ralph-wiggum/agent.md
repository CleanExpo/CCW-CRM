...
_Autonomous task completion through iterative loop with full verification_
  ...
  "user_stories": [
    {
      ...
      "priority": "critical",
      "acceptance_criteria": ["Email validation works", "Password strength checked"],
      "passes": false,
      "attempt_count": 0,
      "depends_on": []
    }
...
The orchestrator can delegate long-running autonomous tasks via `/ralph run`.

### With Long-Running Harness

Ralph follows the same patterns as the CCW agent framework:
...
```
/ralph init    # Initialise plans/ directory with PRD template
/ralph run     # Begin autonomous loop (default 50 iterations)
/ralph run 25  # Run with custom iteration limit
```
