...
vi.mock('@/lib/comms/operational-events', () => ({
  logOperationalEvent: vi.fn().mockResolvedValue(undefined),
}));

vi.mock('@/lib/workflows/workflow-engine', () => ({
  dispatchWorkflowTrigger: vi.fn().mockResolvedValue({ instances: 0 }),
}));
...
function mockTieredCustomer(customerId: string) {
  vi.mocked(prisma.customerPriceTier.findUnique).mockImplementation(((args: {
    where: { customerId?: string };
  }) => {
    if (args.where.customerId !== customerId) return Promise.resolve(null);
...
describe('Pricing endpoints: tiered customer receives contract price', () => {
  ...
  it('POST /api/quotes → line item uses tier price', async () => {
    ...
    const lineItems = createCall.data.lineItems!.create as Array<{
      unitPrice: number;
      lineTotal: number;
    }>;
  ...
  it('POST /api/orders → line item uses tier price', async () => {
    ...
    const lineItems = createCall.data.lineItems!.create as Array<{ unitPrice: number }>;
    expect(lineItems[0].unitPrice).toBe(TIER_PRICE);
  });
