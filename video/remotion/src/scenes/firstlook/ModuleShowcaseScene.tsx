...
export const ModuleShowcaseScene: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{ background: '#0f172a', fontFamily: 'system-ui, -apple-system, sans-serif', padding: '50px 80px', display: 'flex', flexDirection: 'column' }}>
      <div style={{ opacity: headerOpacity, fontSize: 44, fontWeight: 700, color: '#ffffff', marginBottom: 40, textAlign: 'center' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 20, flex: 1 }}>
        {modules.map((mod, i) => {
          ...
          return (
            <div key={i} style={{ opacity, transform: `scale(${scale})`, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '20px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ opacity: footerOpacity, textAlign: 'center', marginTop: 24, fontSize: 28, color: '#60a5fa', fontWeight: 600 }}>
