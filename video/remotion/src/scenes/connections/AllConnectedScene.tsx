...
export const AllConnectedScene: React.FC = () => {
  ...
  const ctaOpacity = interpolate(frame, [360, 420], [0, 1], { extrapolateRight: 'clamp', extrapolateLeft: 'clamp' });
  ...
  return (
    <AbsoluteFill style={{ background: 'linear-gradient(135deg, #0f172a, #052e16)', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '60px 80px' }}>
      <div style={{ opacity: titleOpacity, transform: `scale(${titleScale})`, textAlign: 'center', marginBottom: 40 }}>
        <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'rgba(16,185,129,0.2)', border: '3px solid #10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 36, fontWeight: 800, color: '#10b981' }}>✓</div>
      ...
      <div style={{ display: 'flex', gap: 24, marginBottom: 48 }}>
        {badges.map((b, i) => {
          ...
          return (
            <div key={i} style={{ opacity, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, background: `${b.color}15`, border: `2px solid ${b.color}`, borderRadius: 16, padding: '20px 32px' }}>
              ...
              <div style={{ fontSize: 14, color: '#10b981', fontWeight: 600, letterSpacing: 1 }}>CONNECTED</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ opacity: sectionOpacity, width: '100%', maxWidth: 900, marginBottom: 40 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {automations.map((a, i) => {
            ...
            return (
              <div key={i} style={{ opacity: aOpacity, display: 'flex', alignItems: 'center', gap: 16, fontSize: 22, color: '#e2e8f0', padding: '14px 24px', background: `${a.color}08`, borderLeft: `3px solid ${a.color}`, borderRadius: '0 8px 8px 0' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 6, background: 'linear-gradient(90deg, #96bf48, #10b981, #96bf48)' }} />
    </AbsoluteFill>
  );
};
