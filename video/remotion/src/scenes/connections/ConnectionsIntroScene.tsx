    ...
    <AbsoluteFill style={{ background: '#0f172a', fontFamily: 'system-ui, -apple-system, sans-serif', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 80 }}>
      ...
      <div style={{ display: 'flex', gap: 40, justifyContent: 'center' }}>
        {services.map((s, i) => {
          ...
          return (
            <div key={i} style={{ opacity: sOpacity, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, background: `${s.color}10`, border: `2px solid ${s.color}60`, borderRadius: 16, padding: '28px 48px' }}>
              <div style={{ width: 48, height: 48, borderRadius: '50%', background: s.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 900, color: '#000', letterSpacing: -1 }}>{s.name[0]}</div>
              <span style={{ fontSize: 24, color: '#ffffff', fontWeight: 700 }}>{s.name}</span>
              <span style={{ fontSize: 14, color: s.color, fontWeight: 500, letterSpacing: 1 }}>{s.label.toUpperCase()}</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 4, background: 'linear-gradient(90deg, #96bf48, #10b981)' }} />
    </AbsoluteFill>
  );
};
