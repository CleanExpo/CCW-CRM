...
export const ConnectionStepScene: React.FC<ConnectionStep> = ({
  ...
  const envOpacity = interpolate(frame, [steps.length * 90, steps.length * 90 + 30], [0, 1], {
    extrapolateRight: 'clamp',
    extrapolateLeft: 'clamp',
  });
  const warnOpacity = interpolate(frame, [steps.length * 90 + 60, steps.length * 90 + 90], [0, 1], {
    extrapolateRight: 'clamp',
    extrapolateLeft: 'clamp',
  });
  const screenshotOpacity = interpolate(
    frame,
    [steps.length * 90 + 90, steps.length * 90 + 150],
    [0, 1],
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div>
          <div style={{ fontSize: 16, color: accentColor, fontWeight: 600, letterSpacing: 2 }}>
            {stepNumber}
          </div>
      ...
      <div style={{ display: 'flex', flex: 1, gap: 0 }}>
        ...
        >
          {steps.map((step, i) => {
            const opacity = interpolate(frame, [30 + i * 90, 50 + i * 90], [0, 1], {
              extrapolateRight: 'clamp',
              extrapolateLeft: 'clamp',
            });
            const x = interpolate(frame, [30 + i * 90, 50 + i * 90], [-20, 0], {
              extrapolateRight: 'clamp',
              extrapolateLeft: 'clamp',
            });
            return (
            ...
            {envVars.map((line, i) => (
              <div
                ...
                style={{
                  ...
                  color: '#e2e8f0',
                  lineHeight: 2,
                  borderBottom: i < envVars.length - 1 ? '1px solid #1e293b' : 'none',
                  paddingBottom: i < envVars.length - 1 ? 8 : 0,
                }}
              >
