...
class TechnicalSpecGenerator(BaseAgent):
    ...
    def __init__(self) -> None:
        super().__init__(agent_id=None, name="tech_spec_generator", auto_register=False)
        self.capabilities = [
            "system_architecture",
            "database_design",
            "api_design",
    ...
    async def execute(
        ...
        try:
            ...
            logger.error("Technical specification generation failed", error=str(e))
            return {
                "success": False,
                "error": str(e),
                "task_id": task_id,
    ...
    async def _generate_specification(
        ...
        try:
            ...
            logger.warning("Failed to parse JSON, using fallback", error=str(e))
            return self._parse_markdown_response(content)
...
Return ONLY the JSON, no additional text."""  # noqa: E501
    ...
    def _parse_markdown_response(self, content: str) -> TechnicalSpec:
        ...
        return TechnicalSpec(
            architecture_overview="Technical specification requires manual review. See raw output for details.",  # noqa: E501
