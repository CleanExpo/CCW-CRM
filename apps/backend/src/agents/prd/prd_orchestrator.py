...
class PRDOrchestrator(BaseAgent):
    ...
    def __init__(self) -> None:
        super().__init__(agent_id=None, name="prd_orchestrator", auto_register=False)
        self.capabilities = [
    ...
    async def generate(
        ...
        # task_id tracking handled by orchestrator
        ...
        try:
            ...
            logger.info("Phase 3/5: Generating technical specification...")
            ...
            logger.info("Phase 4/5: Generating test plan...")
            ...
            logger.info("Phase 5/5: Creating implementation roadmap...")
            ...
            documents_generated = []
            if output_dir:
                logger.info("Generating PRD documents...")
            ...
            # output reporting handled by orchestrator
...
"""  # noqa: E501
    ...
    def _generate_tech_spec_markdown(self, tech_spec: TechnicalSpec) -> str:
        ...
        for table in tech_spec.database_schema:
            columns_text = "\n".join(
                f"- **{col['name']}** ({col['type']}): {col.get('description', '')} `{col.get('constraints', '')}`"  # noqa: E501
                for col in table.columns
            )
            indexes_text = "\n".join(f"- {idx}" for idx in table.indexes) if table.indexes else "None"  # noqa: E501
            relationships_text = "\n".join(f"- {rel}" for rel in table.relationships) if table.relationships else "None"  # noqa: E501
...
""")
        ...
        milestones_content = "\n".join(
            f"### {m.name} (Sprint {m.target_sprint})\n{m.description}\n**Deliverables**: {', '.join(m.deliverables)}"  # noqa: E501
