...
async def test_quote_conversion_404():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if not quote_id:
            print(f"❌ Quote created but no ID in response")
        ...
        if response.status_code != 200:
            ...
            print(f"Response: {response.text}")
            return False
        print(f"✅ Quote updated to accepted")
        ...
        if response.status_code == 404:
            print(f"❌ REPRODUCED THE BUG: Got 404 when converting quote")
            print(f"Response: {response.text}")
            ...
            print(f"✅ Quote converted to order successfully")
            order = response.json()
