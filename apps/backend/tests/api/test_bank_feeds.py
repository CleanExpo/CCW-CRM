...
async def test_list_bank_accounts_filters_by_location(
    ...
    """Test bank accounts endpoint with location query param."""
    ...
    assert response.status_code == 200
    data = response.json()
    # Endpoint returns all active accounts; verify it's a list
    assert isinstance(data, list)
...
async def test_create_bank_account(client: AsyncClient, auth_headers: dict, create_test_location):
    ...
    # Account number may be masked (last 4 digits shown) for security
    assert data["account_number"].endswith(payload["account_number"][-4:])
