...
describe('apiClient', () => {
  ...
  describe('GET requests', () => {
    ...
    test('handles 404 errors', async () => {
      ...
      await expect(apiClient.get('/api/not-found')).rejects.toThrow(ApiClientError);
    });
    ...
    test('handles validation errors', async () => {
      server.use(
        http.post(`${BACKEND_URL}/api/products`, () => {
          return HttpResponse.json({ detail: 'SKU already exists' }, { status: 400 });
        })
      );

      await expect(apiClient.post('/api/products', { sku: 'DUPLICATE' })).rejects.toThrow(
        'SKU already exists'
      );
    });
  });
  ...
  describe('PUT requests', () => {
    test('makes successful PUT request', async () => {
      ...
      const result = await apiClient.put<{ id: string; name: string }>(
        '/api/products/1',
        updatedProduct
      );
  ...
  describe('PATCH requests', () => {
    test('makes successful PATCH request', async () => {
      ...
      const result = await apiClient.patch<{ id: string; stock: number }>('/api/products/1', {
        stock: 50,
      });
