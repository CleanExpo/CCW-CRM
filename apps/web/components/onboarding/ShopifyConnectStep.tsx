...
export function ShopifyConnectStep({ onComplete, onSkip, onBack, canGoBack, isOptional }: ShopifyConnectStepProps) {
  ...
  async function handleConnect() {
    ...
    try {
      ...
      // window.location.href = `/api/shopify/oauth?shop=${shopUrl}`;
  ...
  return (
    <div className="space-y-6">
      <Alert>
        <AlertDescription>
          Connect your Shopify store to automatically sync products, inventory, and orders. You can skip
          this step and connect later from Settings.
        </AlertDescription>
      </Alert>
      ...
      <div className="flex justify-between">
        ...
        <div className="flex gap-2">
          ...
          {isConnected && (
            <Button onClick={() => onComplete({ shopUrl, connected: true })}>
              Continue
            </Button>
          )}
        </div>
