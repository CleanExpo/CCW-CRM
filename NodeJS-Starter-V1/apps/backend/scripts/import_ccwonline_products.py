...
def map_category(product_type: str, tags: List[str]) -> str:
    ...
    # Chemicals: Detergents, chemicals (map to BUILDING_MATERIALS as proxy)
        ...
        return "BUILDING_MATERIALS"  # Using as "Supplies"
            ...
            for sp in shopify_products:
                try:
                    ...
                    if not dry_run:
                        ...
                        safe_name = name[:50].encode('ascii', 'replace').decode('ascii')
                        safe_sku = sku[:20].encode('ascii', 'replace').decode('ascii')
                        safe_category = category.encode('ascii', 'replace').decode('ascii')
