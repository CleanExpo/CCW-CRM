...
async def create_stock_transfer(
    ...
    # Broadcast inventory updates for both locations via WebSocket
    ...
    if product:
        # Broadcast for source location
        await broadcast_inventory_update(
            product_id=str(product_id),
            warehouse=transfer_req.from_location,
            data={
                "product_id": str(product_id),
                "product_name": product.name,
                "warehouse": transfer_req.from_location,
                "available": source_stock.available,
                "reserved": source_stock.reserved,
            },
        )

        # Broadcast for destination location
        await broadcast_inventory_update(
            product_id=str(product_id),
            warehouse=transfer_req.to_location,
            data={
                "product_id": str(product_id),
                "product_name": product.name,
                "warehouse": transfer_req.to_location,
                "available": dest_stock.available,
                "reserved": dest_stock.reserved,
            },
        )
...
async def adjust_stock(
    ...
    if product:
        await broadcast_inventory_update(
            product_id=str(product_id),
            warehouse=adjustment_req.location,
            data={
                "product_id": str(product_id),
                "product_name": product.name,
                "warehouse": adjustment_req.location,
