...
from src.db.prd_models import AgentRun, APIUsage, PRD
...
async def generate_prd_background(
    ...
    """Background task to generate PRD using AI orchestrator."""
    try:
        logger.info("Starting PRD generation with AI", prd_id=str(prd_id))

        # Import simple PRD generator
        from src.agents.prd.simple_prd_generator import generate_prd_with_ai

        # Generate PRD using Claude API
        result = await generate_prd_with_ai(
