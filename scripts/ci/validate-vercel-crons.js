...
function validateCrons(config, readSource) {
  ...
  for (const cron of crons) {
    ...
    const marker = STUB_MARKERS.find((m) => source.includes(m));
    if (marker) {
      failures.push(
        `${cronPath}: resolves to a 501 stub (${marker}) — a scheduled job that ` +
          `cannot run. Port it to a Route Handler or remove it from vercel.json.`
      );
    }
  }
