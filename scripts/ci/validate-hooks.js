...
if (!fs.existsSync(HOOKS_FILE)) { fail('hooks/hooks.json not found'); process.exit(1); }

let config;
try { config = JSON.parse(fs.readFileSync(HOOKS_FILE, 'utf8')); pass('hooks.json is valid JSON'); }
catch (e) { fail(`Parse error: ${e.message}`); process.exit(1); }

if (!config.hooks) { fail('Missing top-level "hooks" key'); process.exit(1); }
