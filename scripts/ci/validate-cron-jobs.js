...
if (!fs.existsSync(BOARDROOM)) { console.warn('⚠️  scripts/boardroom/ not found — skipping'); process.exit(0); }

fs.readdirSync(BOARDROOM).filter(f=>f.endsWith('.js')).forEach(f => {
  checked++;
  const content = fs.readFileSync(path.join(BOARDROOM, f), 'utf8');
  const ref = content.match(/UNI-\d+/);
  ref ? console.log(`  ✅ ${f} (${ref[0]})`)
