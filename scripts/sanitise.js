...
function sanitiseExternalContent(content, source = 'unknown') {
  ...
  if (flags.length > 0) {
    console.warn(`[SANITISE] ${source}: ${flags.length} suspicious pattern(s) detected — logged to audit trail`);
    // In production, append to audit log
    try {
      const fs = require('fs');
      const logPath = require('path').join(__dirname, '..', '.claude', 'memory', 'security-audit.log');
