...
const CONNECTIONS_SCRIPT = `To get CCW ERP fully operational, you need to connect four services. Shopify, Xero, Supabase, and your Vercel plus Railway deployment. Each connection takes between two and ten minutes. Follow along, pause at each step, and you'll be live in under thirty minutes.
...
Supabase is your database. Log into supabase dot com and open your project. Click the Settings gear icon in the left sidebar. Under Configuration, click API. You'll find three values here. Your Project URL. Your anon public key. And your service role key. Copy all three. The Project URL and anon key go into Vercel. The service role key goes into Railway. Important: never put the service role key in frontend code. It has full database access. Keep it secret.

Now let's deploy your environment variables. In Vercel, go to your project, click Settings, then Environment Variables. Add NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, and NEXT_PUBLIC_BACKEND_URL. Your backend URL is the URL of your Railway service. In Railway, open your backend service, click the Variables tab, and add DATABASE_URL, SUPABASE_SERVICE_ROLE_KEY, your Xero credentials, and your SECRET_KEY. Both platforms will automatically redeploy once you save.

Time to test. In CCW settings, test your Shopify connection by importing orders. Click Connect Xero to initiate the OAuth flow. Your Supabase connection is confirmed if your dashboard loaded without errors. Visit your Railway health endpoint — it should return 200 OK. Finally, check that your products are visible in the CCW Inventory module. If all checks are green, you're ready.
